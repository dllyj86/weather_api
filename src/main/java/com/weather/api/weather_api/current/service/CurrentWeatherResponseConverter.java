...
public class CurrentWeatherResponseConverter extends Task {
	...
	Logger currentResponseConverterLogger = LogManager.getLogger();
	...
	public boolean run(Map<String, Object> workflowInputData, Map<String, Object> workflowOutputData) throws Exception {
		...
		if (resBody != null) {
			...
			model.setTemperature(
					currentPart.optString(Constants.TEMPERATURE_PROP_KEY) + WeatherDataUtils.getCelsiusSymbol());
			// wind speed
			...
			this.currentResponseConverterLogger
					.error("Key " + Constants.CURRENT_WEATHER_FOR_CITY_RESPONSE_BODY + " didn't get valid JSONObject");
			return false;
		}
